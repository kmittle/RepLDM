...
class FreeScaleSDXLPipeline(DiffusionPipeline, FromSingleFileMixin, LoraLoaderMixin):
    ...
    def __call__(
        ...
        attn_guidance_density=[0]*31 + [1]*15 + [0]*4,
        attn_guidance_scale_decay=None,
        attn_guidance_power_calibrate=None,
        ...
        # attention guidance
        def create_attn_guidance_module(
            latents=latents, num_total_steps=num_inference_steps,
            h=height, w=width,
            attn_type=attn_type,
            guidance_scale=attn_guidance_scale,
            guidance_density=attn_guidance_density,
            guidance_scale_decay=attn_guidance_scale_decay,
            power_calibrate=attn_guidance_power_calibrate,
            guidance_filter=attn_guidance_filter,
        ):
            return AttnGuidance(
                dtype=latents.dtype, device=latents.device, num_total_steps=num_total_steps,
                h=h, w=w,
                attn_type=attn_type,
