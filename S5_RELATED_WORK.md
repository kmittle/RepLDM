...
| PLADIS (2503.07677) | Within one forward pass, contrasts dense and alpha-entmax cross-attention with no training or extra NFE. | This is the closest same-NFE baseline. Use official alpha=1.5 and scale=2.0 on all SDXL cross-attention layers. |
| GAG (2603.02531) | Decomposes PLADIS's sparse-dense contrast and retains a capped retrieval-aligned component. | Use the official SDXL setting lambda=10, eta=15, zeta=0. Projection geometry alone cannot support novelty. |
