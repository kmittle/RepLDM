...
| [SSVAE, 2512.05394](https://arxiv.org/abs/2512.05394) (2025-12-05) | Video-VAE analysis links low spatio-temporal frequencies and a concentrated channel eigenspectrum to diffusability, then regularizes both. | A spectral/coarse-to-fine story needs this latent-spectrum control and must distinguish image SDXL from video-VAE evidence. |
...
### Fresh API audit: geometry, reward sharpness, precision, and off-target drift

On 2026-08-25 UTC, the arXiv API endpoint
`https://export.arxiv.org/api/query?id_list=2603.11509,2603.21175,2604.06916,2607.16291`
returned the v1 records below (the feed reported `updated=2026-08-24T21:44:34Z`).
These are abstract/API mechanism checks, not evidence of peer review or
transfer to SDXL.

| Work (API title and first date) | API-verified mechanism | Novelty that cannot be claimed | Required S8/RL control |
|---|---|---|---|
| [MOG/Auto-MOG, 2603.11509v1](https://arxiv.org/abs/2603.11509v1) (2026-03-12) | Manifold-Optimal Guidance treats CFG as local Riemannian control and gives a closed-form geometry-aware update; Auto-MOG dynamically balances guidance energy without retraining. | Not the first Riemannian/geodesic or manifold-preserving guidance, nor the first dynamic energy/guidance schedule. FRLA's moment retraction and trust cap are constraints, not novelty by themselves. | Run fixed-energy MOG and Auto-MOG at matched U-Net calls, CFG, timesteps, seed, and wall-time budget; retain Euclidean norm-only and isotropic-shell controls. Report tangent/radial components, latent moments, manifold/energy drift, and schedule values. |
| [RSA-FT, 2603.21175v1](https://arxiv.org/abs/2603.21175v1) (2026-03-22) | Reward Sharpness-Aware Fine-Tuning attributes reward hacking to sharp, non-robust reward gradients and flattens them using diffusion-parameter and generated-sample perturbations. | Not the first reward-gradient robustness, sharpness-aware reward optimization, or anti-reward-hacking objective. A higher scalar reward is not evidence of image quality. | If RL uses reward gradients, compare ordinary and flattened/perturbed-gradient training under identical rollouts. For every learned arm, stress parameter and generated-sample perturbations, and report independent native-resolution, patch, color-normalized, and blinded-human witnesses plus the reward-quality gap and gradient sharpness. |
| [Sol-RL, 2604.06916v1](https://arxiv.org/abs/2604.06916v1) (2026-04-08) | Two-stage Speed-of-light RL uses high-throughput NVFP4/FP4 rollouts to form a contrastive candidate pool, then regenerates selected samples and optimizes only in BF16. | Not the first rollout scaling, mixed-precision exploration/optimization split, or FP4 acceleration claim. Any speedup is hardware- and quantization-dependent. | Include BF16-all and FP4-explore/BF16-optimize arms only when an efficiency claim is in scope. Match seeds, candidate-pool size, NFE, policy, and selection rule; measure quantization error, alignment witnesses, convergence, throughput, wall time, peak memory, and amortized training cost. If NVFP4 is unavailable, mark the arm unavailable and make no speed claim. |
| [CIB-Med-1, 2607.16291v1](https://arxiv.org/abs/2607.16291v1) (2026-07-12) | A trajectory-level medical-editing benchmark measures target progression and inversion together with off-target semantic drift over nuisance axes; constrained guidance preserves target progress while bounding collateral change. | Not the first trajectory-level off-target guard, semantic collateral-preservation objective, or constrained target-vs-nuisance guidance. The medical benchmark does not establish SDXL image claims. | Before S8/RL, predeclare target attributes and nuisance axes (for example color, background, pose, texture, identity, count, and layout). Measure per-step and endpoint drift, target progress, inversion, median and 90th-percentile off-target change, and target/off-target correlation. Compare unconstrained FRLA/RL with a drift-constrained arm; an endpoint reward gain with collateral drift fails the semantic gate. |
