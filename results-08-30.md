...
**观察：** 共 12 prompts x 3 seeds x 7 settings = 252 张图，但同一 prompt/seed 的动作分散到不同 GPU。
...
低频、高频和等增益动作更差。HPSv2 的小幅正差值都跨 0。

**结论：** 没有频带稳定优于 `no_ag`，G1 失败，频谱动作不进入 RL。
...
**为什么做：** 检查是否应让不同 prompt 选择不同动作，而不是使用一个全局动作。
...
**结果：** conference expert、raw、tangent、cone 的 TOPIQ 差值分别为 `-0.020052`、`-0.005907`、
...
**结果：** 最好的 reciprocal semantic action TOPIQ 差值只有 `+0.000509`（CI `[-0.000571,+0.001771]`），
...
**观察：** 11 x 2 x 7 = 154 张同卡配对图；drift 动作严重退化，stochastic correction 的 correction/update ratio 平均约
...
**结论：** 只能说明 sampler 之间有差异，不能说明 latent correction 有效；所有 scheduler action 禁止用于方法选择。
...
**为什么做：** 修正坐标后，用固定的 scheduler-native 低/中/高频和 Laplacian 动作重新检查是否存在最基本的静态 headroom。
...
**结论：** evaluator 返回 `null_route`；固定动作族不能进入 validation、learned renderer、蒸馏或 RL。
...
目前没有正式 RL、OPD 或 latent-renderer 训练结果。所有候选路线都出现以下两种情况之一：分数提高但伴随明显的颜色、对比度、锐度、clipping 或 saturation 副作用；或者副作用被压住后，质量提升同时消失。当前没有一个固定动作在已有数据上提供足够、可靠的静态 headroom。

因此，下一步不能靠调 reward、堆 controller 或直接训练 RL 来掩盖基础动作失败。若继续研究，必须先重新登记一个与 scheduler 坐标一致、方向来源实质不同的算子，在全新的 prompt-disjoint 数据上通过 `+0.005` TOPIQ、CI、HPSv2/CLIP 非劣和像素保护门槛；只有固定动作先通过，才有理由进入 renderer、蒸馏或 RL。以上小规模结果不能替代最终的 1,000/2,000 prompts x 3 seeds 主流 benchmark。
