...
_RENAME_NOREPLACE = 1
_PUBLICATION_RECONCILIATION_ATTEMPTS = 8
_FD_SAFETY_RESERVE = 64
_GENERATION_PINNED_FILE_COUNT = 1 + (2 * contract.TOTAL_TASK_COUNT) + 5
_AUDIT_PINNED_FILE_COUNT = 2 + 1 + (2 * contract.TOTAL_TASK_COUNT) + 4 + 1
...
class _RuntimeEvidenceCapture:
    ...
    def _close_stderr_descriptor_number(
        ...
        if expected_identity is None:
            try:
                self._stderr_descriptor_identity(descriptor)
            except OSError as exc:
                if exc.errno == errno.EBADF:
            ...
            errors.append(
                RuntimeError(
                    "stderr descriptor ownership identity is unavailable; refusing to close"
                )
            )
            return errors, False, None
        else:
    ...
    def _close_stderr_descriptor(
        ...
        if released:
            ...
            # Keep the descriptor number and identity until its private anchor
            # is also released.  This preserves enough ownership state for a
            # later retry when anchor close is cancelled after the descriptor
        ...
        for descriptor, expected_identity in tuple(self._stderr_guarded_fds.items()):
            ...
            if released:
                ...
                if anchor_released:
                    ...
                    self._stderr_guarded_fds[descriptor] = expected_identity
            else:
...
def _require_fd_headroom(required: int, label: str) -> None:
    if isinstance(required, bool) or not isinstance(required, int) or required < 0:
        raise ValueError("required file-descriptor headroom must be nonnegative")
    soft_limit, _ = resource.getrlimit(resource.RLIMIT_NOFILE)
    if soft_limit == resource.RLIM_INFINITY:
        return
    open_count = _open_descriptor_count()
    available = max(0, int(soft_limit) - open_count)
    if available < required:
        raise RuntimeError(
            f"{label} requires at least {required} free file descriptors; "
...
def _open_pinned_directory_chain(
    root: Path,
...
def _verify_directory_chain(
    root: Path,
    root_descriptor: int,
    links: Sequence[tuple[int, str, int, str]],
    label: str,
) -> None:
...
def _link_descriptor(
    ...
    procfs = os.stat(source, follow_symlinks=True)
    if (
        not stat.S_ISREG(held.st_mode)
        or not stat.S_ISREG(procfs.st_mode)
        or _object_identity(held) != _object_identity(procfs)
    ):
        raise RuntimeError("procfs publication source differs from the held inode")
    os.link(
        source,
        name,
...
def _link_with_positive_reconciliation(
    descriptor: int,
    directory_descriptor: int,
    name: str,
    identity: tuple[int, int],
) -> Optional[tuple[BaseException, Any]]:
    """Link once, retrying ambiguous negative observations until state is positive."""
    ...
    for attempt in range(_PUBLICATION_RECONCILIATION_ATTEMPTS):
        ...
        try:
            ...
            remember(observe_error)
            back_off(attempt)
            continue
        if stat.S_ISREG(observed.st_mode) and _object_identity(observed) == identity:
            return first_error
        original, original_traceback = first_error
        raise original.with_traceback(original_traceback)

    unresolved = AtomicPublicationIndeterminate(
        f"atomic publication outcome remained indeterminate for {name}"
    )
    unresolved.target_name = name
    unresolved.source_identity = identity
    if first_error is not None:
        unresolved.first_error = first_error[0]
    if last_error is not None:
        raise unresolved from last_error
    raise unresolved
...
def _pread_descriptor_bytes(descriptor: int) -> bytes:
    chunks = []
    offset = 0
    while True:
...
def _stable_regular_snapshot(
    ...
    """Read one descriptor snapshot bracketed by inode and path metadata."""

    last_error: Optional[BaseException] = None
    for _ in range(_PUBLICATION_RECONCILIATION_ATTEMPTS):
        try:
    ...
    raise RuntimeError(f"{label} did not provide a stable descriptor snapshot")
...
class _PublishedArtifacts:
    ...
    def add_exact(
        ...
        try:
            ...
            if (
                (metadata[0], metadata[1]) != identity
                or raw != payload
            ):
...
def _remove_matching_entry(
    ...
    observed = _entry_metadata(directory_descriptor, name)
    if observed is None:
        return
    if (
        not stat.S_ISREG(observed.st_mode)
        or _object_identity(observed) != identity
    ):
        raise RuntimeError(f"cannot roll back foreign atomic output {name}")
    ...
    try:
        ...
        quarantined = _entry_metadata(directory_descriptor, quarantine_name)
        if quarantined is None:
            if _entry_metadata(directory_descriptor, name) is None:
                raise RuntimeError(
                    f"rollback move outcome could not be reconciled for {name}"
                ) from rename_error
            raise
    quarantined = _entry_metadata(directory_descriptor, quarantine_name)
    if quarantined is None:
        raise RuntimeError(f"rollback quarantine disappeared for {name}")
    if (
        not stat.S_ISREG(quarantined.st_mode)
        or _object_identity(quarantined) != identity
    ):
        os.fsync(directory_descriptor)
        raise RuntimeError(
            f"foreign atomic output was preserved as {quarantine_name}"
        )
    if _entry_metadata(directory_descriptor, name) is not None:
        os.fsync(directory_descriptor)
        raise RuntimeError(
            f"foreign atomic output appeared while rolling back {name}; "
            f"the owned inode remains as {quarantine_name}"
        )
    try:
        os.unlink(quarantine_name, dir_fd=directory_descriptor)
    except OSError as unlink_error:
        if _entry_metadata(directory_descriptor, quarantine_name) is not None:
            raise unlink_error
    os.fsync(directory_descriptor)
    if _entry_metadata(directory_descriptor, quarantine_name) is not None:
        raise RuntimeError(f"atomic output rollback did not remove {name}")
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            if (
                deferred_link_error is not None
                and not isinstance(deferred_link_error[0], OSError)
            ):
                link_error, link_traceback = deferred_link_error
        ...
        if (
            ...
            or _object_identity(published) != temporary_identity
            or published.st_size != len(payload)
            or _read_descriptor_bytes(descriptor) != payload
        ):
            if _object_identity(published) != temporary_identity:
                published_identity = None
                preserve_temporary = True
    ...
    if primary_error is None and result is not None:
        try:
            ...
            if (
                ...
                or _object_identity(durable) != result
                or durable.st_size != len(payload)
                or _read_descriptor_bytes(descriptor) != payload
            ):
                raise RuntimeError(
                    "atomic output changed during staging cleanup"
                )
        except BaseException as exc:
            primary_error = (exc, exc.__traceback__)
            try:
                _remove_matching_entry(directory_descriptor, name, result)
            except BaseException as rollback_error:
                cleanup_errors.append(rollback_error)
                preserve_temporary = True
            result = None
    ...
    if cleanup_errors and any(
        ...
        raise error
    if result is None:
        raise RuntimeError("atomic publication completed without an inode identity")
    return result
...
def _prepare_atomic_output_at(
    ...
    *,
    ...
    if not name or Path(name).name != name:
        raise ValueError("atomic output name must be one safe basename")
    target_directory = os.fstat(directory_descriptor)
    staging_directory = os.fstat(staging_directory_descriptor)
    if not stat.S_ISDIR(target_directory.st_mode):
        raise ValueError("atomic output parent descriptor must be a directory")
    if not stat.S_ISDIR(staging_directory.st_mode):
        raise ValueError("atomic staging descriptor must be a directory")
    if target_directory.st_dev != staging_directory.st_dev:
        raise ValueError("atomic output and staging directories must share a filesystem")
    if ownership_candidates is not None and ownership_candidates:
        raise ValueError("atomic ownership candidate list must start empty")
...
def _discard_prepared_atomic_output(
    prepared: _PreparedAtomicOutput,
...
def _commit_prepared_terminal_impl(
    ...
    try:
        raw, observed = _stable_regular_snapshot(
            ...
            prepared.target_directory_descriptor,
            prepared.target_name,
            f"terminal receipt {prepared.target_name}",
        )
        if (
            (observed[0], observed[1]) != prepared.identity
            or raw != prepared.payload
        ):
            raise RuntimeError(
                f"terminal receipt {prepared.target_name} changed during commit"
            )
...
def _run_authorized_engineering_generation(
    ...
    expected_output = repo_root.joinpath(*Path(OUTPUT_DIR).parts)
    if output != expected_output:
        raise ValueError("validated engineering output path differs from the contract")
    if os.path.lexists(output):
        raise FileExistsError(
            "adaptive-oracle engineering output already exists; resume/retry is forbidden"
        )
    _require_fd_headroom(
        _GENERATION_PINNED_FILE_COUNT + _FD_SAFETY_RESERVE,
        "adaptive-oracle engineering generation",
    )
    completed = 0
    owned_output = False
    root_descriptor = -1
    output_parent_links: list[tuple[int, str, int, str]] = []
    ...
    try:
        ...
        try:
            output_descriptor = _claim_pinned_directory_at(
                output_parent_descriptor,
                output.name,
        ...
        _verify_directory_inventory(
            ...
            "pre-terminal engineering output",
        )
        _verify_directory_inventory(
            records_descriptor,
            expected_record_names,
        ...
        prepared_success = _prepare_terminal_json(
            output / "success.json",
            success,
            directory_descriptor=output_descriptor,
            staging_directory_descriptor=output_parent_descriptor,
        ...
        try:
            ...
            publications.verify()
            _commit_prepared_terminal(prepared_success)
        except BaseException as exc:
            _discard_prepared_atomic_output(prepared_success, exc)
            raise
        return success
    except BaseException as exc:
        ...
        if not cancellation_seen and model_stage is not None and owned_output:
            ...
            try:
                if output_descriptor >= 0 and not _entry_exists(
                    output_descriptor, model_stage_evidence_path.name
                ):
                    _verify_directory_chain(
                        repo_root,
                        root_descriptor,
