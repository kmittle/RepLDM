...
所有正式方法实验必须完整生成这 3,200 行，不能用某个风格、随机子集或历史
小型 prompt 集代替。官方清单包含重复文本，必须保留每个官方行并用风格和行号
区分，不能自行去重。少量 prompt 只允许检查导入、显存、输出格式和数值错误；
这类 smoke 不评分、不选择实验设置，也不进入结果表。若一个实验使用多个 seed，
每个 seed 都必须覆盖完整 3,200 行，并让所有实验设置使用完全相同的 seed。
