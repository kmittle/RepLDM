...
> 截止提交：主 pilot `63184bc`，幅度 follow-up `c385223`。本文只记录当前产物可复核的事实。

## 0. 审稿结论

**G1（频谱 headroom）失败，G3（内容自适应 headroom）失败，因此不应训练 RL 控制器。** 频谱分配相对统一标量大幅减轻了 TOPIQ-NR 损伤，但没有优于 `no_ag`。把幅度从 `0.004` 降到 `0.001` 只会收敛回 `no_ag`，未出现内部质量最优点。逐 prompt 动作选择在留出 seed 上也没有可辨认收益，并仍增加饱和度。
