...
> 截止提交：频谱 pilot `63184bc`，幅度 follow-up `c385223`，固定矩 pilot `65ba734`。本文只记录当前产物可复核的事实。

## 0. 审稿结论

**G1（频谱 headroom）、G3（内容自适应 headroom）和 S2（固定矩切空间）均失败，因此这些动作空间都不应训练 RL 控制器。** 频谱分配相对统一标量大幅减轻了 TOPIQ-NR 损伤，但没有优于 `no_ag`。把幅度从 `0.004` 降到 `0.001` 只会收敛回 `no_ag`，未出现内部质量最优点。固定矩约束成功消除了饱和度漂移，但剩余空间方向仍未带来质量或偏好收益。
...
两轮有效实验的每个 `(prompt, seed)` 动作区块均固定在同一 GPU；4 块 GPU 分别生成 99/90 条记录。756 条记录均有可解码的 1024² PNG 和完整指标。follow-up 与 pilot 重复的 4 个动作共 144 对 PNG，其 SHA-256 和全部评分逐值相同，说明续跑可复现，但这 144 对不是新增独立证据。
...
| G5 transfer | 不执行 | 没有候选 winner 可迁移 |
